...
output "logout_url"{
   value = "https://${aws_cognito_user_pool_domain.domain.domain}.auth.us-east-1.amazoncognito.com/logout?client_id=${aws_cognito_user_pool_client.myclient.id}&response_type=token&redirect_uri=${urlencode("https://${data.terraform_remote_state.s3.outputs.bucket_name}.s3.amazonaws.com/index.html")}"
}